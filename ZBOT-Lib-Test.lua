...
local ZBOT = useStudio and script.Parent:FindFirstChild('ZBOT') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned

repeat
	if ZBOT:FindFirstChild('Build') and ZBOT.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('ZBOT | Build Mismatch')
		print('ZBOT may encounter issues as you are running an incompatible interface version ('.. ((ZBOT:FindFirstChild('Build') and ZBOT.Build.Value) or 'No Build') ..').\n\nThis version of ZBOT is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, ZBOT = ZBOT, useStudio and script.Parent:FindFirstChild('ZBOT') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(ZBOT)
	ZBOT.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	ZBOT.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == ZBOT.Name and Interface ~= ZBOT then
			Interface.Enabled = false
			Interface.Name = "ZBOT-Old"
		end
	end
elseif not useStudio then
...
if ZBOT.AbsoluteSize.X < minSize.X and ZBOT.AbsoluteSize.Y < minSize.Y then
...
local Main = ZBOT.Main
local MPrompt = ZBOT:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = ZBOT:FindFirstChild('Drag')
local dragInteract = dragBar and dragBar.Interact or nil
local dragBarCosmetic = dragBar and dragBar.Drag or nil

ZBOT.DisplayOrder = 100
...
local Notifications = ZBOT.Notifications

local SelectedTheme = ZBOTLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = ZBOTLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	ZBOT.Main.BackgroundColor3 = SelectedTheme.Background
	ZBOT.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	ZBOT.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	ZBOT.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	ZBOT.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	ZBOT.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	ZBOT.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	ZBOT.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	...
	for _, text in ipairs(ZBOT:GetDescendants()) do
...
local function getIcon(name : string)
	...
	local iconData = not useStudio and game:HttpGet('https://raw.githubusercontent.com/SiriusSoftwareLtd/ZBOT/refs/heads/main/icons.lua')
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, Flag in pairs(ZBOTLibrary.Flags) do
		...
		if FlagValue then
			...
			warn("ZBOT | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--ZBOTLibrary:Notify({Title = "ZBOT Flags", Content = "ZBOT was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(ZBOTLibrary.Flags) do
...
function ZBOTLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('ZBOT | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			ZBOTLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show ZBOT'.", Duration = 7, Image = 4400697855})
		else
			ZBOTLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K.", Duration = 7, Image = 4400697855})
		end
	end
...
function ZBOTLibrary:CreateWindow(Settings)
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				ZBOTLibrary:Notify({Title = 'Build Mismatch', Content = 'ZBOT may encounter issues as you are running an incompatible interface version ('.. ((ZBOT:FindFirstChild('Build') and ZBOT.Build.Value) or 'No Build') ..').\n\nThis version of ZBOT is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "ZBOT"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "ZBOT Interface Suite" then
		LoadingFrame.Version.Text = "ZBOT UI"
	end
	...
	if Settings.Discord and not useStudio then
		...
		if isfile and not isfile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"ZBOT RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("ZBOT | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZBOT specific development.')
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						...
						ZBOTLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ZBOT | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZBOT specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= ZBOTLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					ZBOTLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			ZBOT.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("ZBOT | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZBOT specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					ZBOTLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("ZBOT | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with ZBOT specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					ZBOTLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("ZBOT | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with ZBOT specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ZBOTLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= ZBOTLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ZBOT | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZBOT specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					ZBOTLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("ZBOT | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with ZBOT specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					ZBOTLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			ZBOTLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			ZBOTLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
...
function ZBOTLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function ZBOTLibrary:IsVisible(): boolean
	return not Hidden
end

function ZBOTLibrary:Destroy()
	ZBOT:Destroy()
end
...
function ZBOTLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				ZBOTLibrary:Notify({Title = "ZBOT Configurations", Content = "We couldn't enable Configuration Saving as you are not using file supported software.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			ZBOTLibrary:Notify({Title = "ZBOT Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('ZBOT Configurations Error | '..tostring(result))
			ZBOTLibrary:Notify({Title = "ZBOT Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
end
...
if useStudio then
	...
	local Window = ZBOTLibrary:CreateWindow({
		Name = "ZBOT Example Window",
		LoadingTitle = "ZBOT Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using ZBOT may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like ZBOT to get the key from
	...
	--ZBOTLibrary:Notify({Title = "ZBOT Interface", Content = "Welcome to ZBOT. These - are the brand new notification design for ZBOT, with custom sizing and ZBOT calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(ZBOTLibrary.Theme) do
